...
fn is_valid_field(field: &[u8]) -> bool {
    ...
    match str::from_utf8(key).unwrap() {
        "byr:" => if let Ok(v) = str::from_utf8(value).unwrap().parse::<u16>() {
            return v >= 1920 && v <= 2002;
        }
        "iyr:" => if let Ok(v) = str::from_utf8(value).unwrap().parse::<u16>() {
            return v >= 2010 && v <= 2020;
        }
        "eyr:" => if let Ok(v) = str::from_utf8(value).unwrap().parse::<u16>() {
