...
lazy_static! {
    static ref RE: Regex = Regex::new(
        r"^(?P<name>.+): (?P<n1>\d+)-(?P<n2>\d+) or (?P<n3>\d+)-(?P<n4>\d+)$"
    ).unwrap();
}
...
fn parse<'t>(captures: &Captures<'t>, key: &str) -> usize {
    std::str::from_utf8(
        captures.name(key).unwrap().as_bytes()
    ).unwrap().parse().unwrap()
}
...
fn parse_input(input: &str) -> Input {
    ...
    for line in group_iter.next().unwrap().1 {
        ...
        if name.starts_with(b"departure") { interesting_rules.push(name_hash) }
    ...
    group_iter.next();
    let this_ticket = parse_ticket(
        group_iter.next().unwrap().1.skip(1).next().unwrap()
    );

    group_iter.next();
    let nearby_tickets = group_iter.next().unwrap().1
        .skip(1)
        .map(parse_ticket)
        .collect();

    (rules, all_rules, interesting_rules, this_ticket, nearby_tickets)
}
...
fn part1((rules, _, _, _, nearby_tickets): &Input) -> usize {
    nearby_tickets
        .iter()
        .flat_map(|ticket|
            ticket.iter().filter(|num| rules[**num].is_empty())
        )
        .sum()
}
...
fn clean_non_ambiguous(possible_names: &mut Vec<Vec<u64>>, name: u64) {
    ...
    for this_names in possible_names.iter_mut() {
        if this_names.len() <= 1 { continue }
        this_names.retain(|&n| n != name);
        if this_names.len() == 1 { next.push(this_names[0]); }
    }

    for n in next { clean_non_ambiguous(possible_names, n); }
}
...
fn part2((rules, all_rules, interesting_rules, this_ticket, nearby_tickets): &Input) -> usize {
    let valid_tickets = nearby_tickets.iter().filter(|ticket|
        ticket.iter().all(|&num| !rules[num].is_empty())
    );
    ...
    for ticket in valid_tickets {
        for (i, num) in ticket.iter().enumerate() {
            if possible_names[i].len() <= 1 { continue }
